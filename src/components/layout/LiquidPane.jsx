...
const LiquidPane = ({ isOpen, onClose, children, title, zIndex = 100 }) => (
    <div className={`fixed inset-0 flex flex-col justify-end transition-all duration-500 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`} style={{ zIndex }}>
        ...
        <div className={`relative w-full bg-[#FAFAFA] z-10 overflow-hidden flex flex-col shadow-2xl transform transition-all duration-800 cubic-bezier(0.32, 0.72, 0, 1) ${isOpen ? 'translate-y-0 h-[92vh]' : 'translate-y-full h-0'}`} style={{ borderTopLeftRadius: isOpen ? '24px' : '100%', borderTopRightRadius: isOpen ? '24px' : '100%' }}>
